	...
	private ArrayList<Bug> parseXML() {
		...
		try {
			...
			for (int i = 0; i < tables.getLength(); i++) {
				...
				for (int j = 0; j < columns.getLength(); j++) {
					...
					switch (columnName) {
						...
						case "description":
							...
							break;
						case "report_time":
							bug.setOpenDate(makeTime(columnElement.getTextContent()));
							break;
						case "commit_timestamp":
							bug.setFixDate(makeTime(columnElement.getTextContent()));
							break;
						case "files":
				...
				// Split the "files" field by newline and add each file to the bug
				if (!files.isEmpty()) {
					String[] fixedFiles = files.split("\n");
